    ...
    for(int y = 0; y < map->get_height(); y++) {
        for(int x = 0; x < map->get_width(); x++) {
            if(map->get_node(x,y)->get_cost()==-1) {
                map->get_node(x,y)->set_isVisited(-1); // block
            }//if
            else
                map->get_node(x,y)->set_isVisited(0); // visible nodes
        }//for x
    }//for y
    // debug function: check whether isVisited function is correct
    for(int y = 0; y < map->get_height(); y++) {
        for(int x = 0; x < map->get_width(); x++) {
            cout<<map->get_node(x,y)->get_isVisited()<<"\t";
        }//for x
        cout<<endl;
    }//for y
}
...
void Algorithm::Bidirection::forward() {
    for(int curConnection = 0; curConnection<number; curConnection++) {
            ...
            for(int y = 0; y < map->get_height();y++) {
                for(int x = 0; x < map->get_width();x++) {
                    if(map->get_node(x,y)->get_cost()!=-1) {
                        ...
                        map->get_node(x,y)->set_isVisited(0); //reset isVisited to 0
                    }//if
                }//for x
            }//for y
        }// if curConnection
        ...
        map->get_node(source.at(curConnection)->get_x(), source.at(curConnection)->get_y())->set_isVisited(1);
        map->get_node(sink.at(curConnection)->get_x(), sink.at(curConnection)->get_y())->set_isVisited(2);
        
        bool flag = false; // this parameter is to determine whether source and sink expansion meets. If yes then flag==true, vice versa.
        
        // everything here is same to the corresponding part of lee.cc, the only difference is that I copied all temporary parameters into two parts. One is for source expansion and another one is for sink expansion
        int valueSource = 0;
        int valueSink = 0;
        ...
        while(flag != true) {
//            flag = expand(source.at(curConnection),sourceInfo,1);
            int f1Source=0;
            int f1Sink=0;
            int f2Source=0;
            int f2Sink=0;
            ...
            while(!q1Source.empty()) { // Source black box first
                ...
                for(int j = 0; j<tempSource->connections_size(); j++) {
                    ...
                    if(map->get_node(neiborSource->get_x(),neiborSource->get_y())->get_isVisited() == 2) { // if this node is visited by sink expansion
                        flag = true;
                        this->meetPointsSource.push_back(tempSource);
                        this->meetPointsSink.push_back(neiborSource);
                        break;
                    } // trace back will be done later
                    if(map->get_node(neiborSource->get_x(),neiborSource->get_y())->get_isVisited() == 0) {
                        map->get_node(neiborSource->get_x(),neiborSource->get_y())->set_isVisited(1); // visited by source expansion, therefore set as 1
            ...
            while(!q1Sink.empty()) { // Sink black box second
                ...
                for(int j = 0; j<tempSink->connections_size(); j++) {
                    ...
                    if(map->get_node(neiborSink->get_x(),neiborSink->get_y())->get_isVisited() == 1) { // if this node is visited by source expansion
                        flag = true;
                        this->meetPointsSink.push_back(tempSink);
                        this->meetPointsSource.push_back(neiborSink);
                        break;
                    } // trace back will be done later
                    if(map->get_node(neiborSink->get_x(),neiborSink->get_y())->get_isVisited() == 0) {
                        map->get_node(neiborSink->get_x(),neiborSink->get_y())->set_isVisited(2); // visited by sink expansion, therefore set as 2
...
//                        map->display_map();
            ...
            map->display_map();
//            flag = expand(sink.at(curConnection),sinkInfo,2);
        }//whileq2sink
...
void Algorithm::Bidirection::traceback(int curConnection) {
    ...
    bool flag=false;
    cout<<this->source.at(curConnection)->get_y()<<this->source.at(curConnection)->get_x()<<this->source.at(curConnection)->get_cost()<<this->source.at(curConnection)->get_isVisited()<<endl;
    cout<<this->sink.at(curConnection)->get_y()<<this->sink.at(curConnection)->get_x()<<this->sink.at(curConnection)->get_cost()<<this->sink.at(curConnection)->get_isVisited()<<endl;
    ...
    while(curNode != this->source.at(curConnection)) {
        for(int i = 0; i < curNode->connections_size(); i++) {
            ...
            if(map->get_node(nxtNode->get_x(),nxtNode->get_y())->get_isVisited() == 1 && ((nxtNode->get_cost() == curNode ->get_cost()-1) || (nxtNode->get_cost() == -2))) {
    ...
    while(curNode != this->sink.at(curConnection)) {
        for(int i = 0; i < curNode->connections_size(); i++) {
            ...
            nxtNode = curNode->connections_at(i) -> get_end(curNode);
            // visited by source, and less distance to the source
            if(map->get_node(nxtNode->get_x(),nxtNode->get_y())->get_isVisited() == 2 && ((nxtNode->get_cost() == curNode ->get_cost()-1) || (nxtNode->get_cost() == -3))) {
                PathSegment* new_segment = new PathSegment(curNode->get_coord(),nxtNode->get_coord());
    ...
    PathSegment* conc = new PathSegment(meetPointsSink.at(curConnection)->get_coord(),meetPointsSource.at(curConnection)->get_coord());
...
/*void Algorithm::Bidirection::display() {
    cout<<"Results of using Bidirection\n";
    for(i = 0; i<paths.size(); i++) {
        cout<<"Path for the source sink pair: "<<source.at(i)->display_node()<<" and "<<sink.at(i)->display_node()<<endl;
        for(j = 0; j<paths.at(i)->size(); j++) {
...
/*Algorithm::Bidirection::expand(Node *n, ExpansionInfo typeInfo, int typeVisit) {
