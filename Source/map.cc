...
Utilities::Map::Map(int width, int height) {
    for(int y = 0; y < height; y++) {
	    ...
	    if (x > 0) {
                Edge* west = new Edge(new_node,temp_row.at(temp_row.size()-1)); // Edge(new_node, temp_row.at(x-1))? also, need to set head and tail?
		new_node->add_connection(west);
	     }
	     if (y > 0) {
		Edge* north = new Edge(new_node,map.at(y-1).at(x)); // this is good
